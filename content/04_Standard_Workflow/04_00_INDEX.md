# 4) Standard Workflow — “как мы работаем с агентами” (единый процесс)

- **Owner**: DevEx / Engineering Managers (TBD)
- **Статус**: Draft
- **Last reviewed**: 2026-01-15
- **Уровень**: Basic
- **Для ролей**: Dev / TL / QA / SRE / Analyst

## Содержание раздела

- [4.1 Spec → Plan → Implement → Verify → PR](04_01_Spec_Plan_Implement_Verify_PR.md)
- [4.2 Definition of Done для AI‑изменений](04_02_Definition_of_Done_AI.md)
- [4.3 Шаблоны артефактов: Spec/ADR/TestPlan](04_03_Artifact_Templates.md)
- [4.4 Управление контекстом](04_04_Context_Management.md)
- [4.5 Многоагентные сценарии и параллельная работа](04_05_Multi_Agent_Parallel.md)

